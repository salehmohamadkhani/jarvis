...
export function PlannerProvider({ children }) {
  ...
  const value = useMemo(() => {
    ...
    const addMeeting = async (input) => {
      try {
        let meetingProjectId = input.projectId != null ? String(input.projectId) : null
        if (input.projectName && (!meetingProjectId || !UUID_REGEX.test(meetingProjectId.trim()))) {
          const found = state.projects.find(
            (p) => p.name && input.projectName && p.name.toLowerCase().includes(input.projectName.toLowerCase())
          )
          if (found) meetingProjectId = found.id
        }
        if (meetingProjectId && !UUID_REGEX.test(String(meetingProjectId).trim())) meetingProjectId = null
        const newMeeting = await meetingsApi.create({
          projectId: meetingProjectId || null,
          title: input.title,
