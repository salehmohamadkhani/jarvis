...
app.post('/api/chatgpt-proxy', async (req, res) => {
  try {
    console.log('ChatGPT proxy called, body:', JSON.stringify(req.body).substring(0, 100));
    ...
    if (!response.ok) {
      ...
      console.error('OpenAI API error:', response.status, text.substring(0, 200));
