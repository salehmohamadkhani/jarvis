    ...
    <DSSection title="Finance">
      ...
      {lastTx ? (
        <div className="ds-last-tx">
          Last transaction: {lastTx.type} — {Number(lastTx.money?.amount || lastTx.amount || 0).toLocaleString()} — {lastTx.date ? new Date(lastTx.date).toLocaleDateString() : '—'}
        </div>
      ) : (
        <div className="ds-empty">No transactions recorded.</div>
      )}
    </DSSection>
  )
}
