...
export default function TodayMeetings() {
  ...
  return (
    <DSSection title="Meetings Today">
      {meetings.length === 0 ? (
        <div className="ds-empty">No meetings scheduled for today.</div>
      ) : (
              ...
              <li key={m.id} className="ds-list-item">
                ...
                <div className="ds-list-item-meta">
                  ...
                  {m.participants && m.participants.length > 0 && (
                    <span className="ds-badge">{typeof m.participants[0] === 'object' ? m.participants[0].name || m.participants[0].email : m.participants[0]}</span>
                  )}
                  {m.htmlLink && (
                    <a href={m.htmlLink} target="_blank" rel="noopener noreferrer" className="ds-badge ds-badge-link">تقویم</a>
                  )}
                </div>
              </li>
            )
